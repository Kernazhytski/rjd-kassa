...
import { HttpTransferException } from 'exceptions/http-transfer.exception';
...
export class AppExceptionFilter implements ExceptionFilter {
  async catch(
    ...
    if (exception instanceof HttpTransferException) {
      ...
      responseBody.meta = errors.message;
      response.status(errors.statusCode || 500).json(responseBody);
      return;
    }
